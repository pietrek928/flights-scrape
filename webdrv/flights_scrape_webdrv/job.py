...
async def _process_job(process_job, worker, job, worker_jobs_limit):
    worker_jobs_limit -= 1
    return worker, await process_job(worker, job), worker_jobs_limit
    ...
    try:
        ...
        while jobs or working_jobs:
            ...
            while len(working_jobs) < max_workers and jobs:
                working_jobs.add(create_task(
                    _process_job(process_job, await make_worker(), jobs.pop(), max_worker_jobs)
                ))
        for worker, worker_jobs_limit in working_jobs:
            await worker.close()
    finally:
        if save_jobs_fname:
            save_jobs(save_jobs_fname, jobs)
